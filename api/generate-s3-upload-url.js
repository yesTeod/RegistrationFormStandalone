...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (idSide !== 'front' && idSide !== 'back' && idSide !== 'selfie' && idSide !== 'face_liveness_check') {
      return res.status(400).json({ success: false, error: "idSide must be 'front', 'back', 'selfie', or 'face_liveness_check'" });
    }
    ...
    try {
      ...
      } else { // idSide === 'face_liveness_check'
        updateQuery.$set.faceLivenessVideoS3Key = uniqueFileName;
      }
